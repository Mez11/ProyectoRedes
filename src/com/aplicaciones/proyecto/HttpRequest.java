...
public class HttpRequest {
	...
	private String getStringHeader( boolean debug, DataInputStream data ) throws IOException{
		...
		if( debug ){
			//Loggers
			log.debug("[HttpRequest] Start of the request:::::");
			//System.out.println( "[HttpRequest] Start of the request:::::");
			log.debug(aux, "aux");
			//System.out.println( aux );
			log.debug("[HttpRequest] End of the request:::::");
	...
	private boolean processFirstLine( String line, boolean verbose ){
		int space = 0;
		if( verbose ){
			log.info(line, "[HttpRequest] First line: ");
			//System.out.println( "[HttpRequest] First line: " + line);
		}
		
		if( line == null || line.isEmpty() ){
			log.error("[HttpRequest] There are no lines in the HTTP header...");
		...
		if( verbose ){
			...
			log.debug("[HttpRequest] HttpVersion: ", httpVersion);
	...
	private void processURI( boolean verbose ){
		...
		if( verbose )
			//log.info(requestURI, "[HttpRequest] Analyzing for params: [" + requestURI + "]" );
			log.info(requestURI, "[HttpRequest] Analyzing for params:");
		...
		if( position == -1 ){
			...
			log.info(fileName, "[HttpRequest] No params found. fileName: ");
		...
		if( verbose ){
			log.info(fileName,  "[HttpRequest] fileName: ");
	...
	private void processParams( String paramString, boolean verbose ){
		...
		if( verbose ){
			log.info("[HttpRequest] Params found: ");
			//System.out.println( "[HttpRequest] Params found: " );
			for( Map.Entry<String, String> entry : params.entrySet( ) ){
				System.out.println( "\t" + entry.getKey() + " - " + entry.getValue( ) );
			}
			log.info("[HttpRequest] end params");
	...
	public boolean substractInfo( boolean verbose ){
		...
		if( verbose )
			...
			//log.i
			log.info("[HttpRequest] Starting to analyze the request..." );
